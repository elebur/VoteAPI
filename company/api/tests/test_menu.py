...
from api.tests.tools import auth_client, de_json, get_jwt_for_user, AUTH_REQUIRED_401
...
def test_create_without_items(user, client, restaurant):
    ...
    assert resp.text == '''{"details":"'items' is the required parameter. It can\'t be null or an empty array"}'''
...
def test_create_with_item_without_title(client, user, restaurant):
    ...
    assert resp.text == '{"details":{"items":[{"title":["This field is required."]},{},{},{},{}]}}'
    assert resp.status_code == 400
...
def test_create_with_item_without_description(client, user, restaurant):
    ...
    assert resp.text == '{"details":{"items":[{"description":["This field is required."]},{},{},{},{}]}}'
    assert resp.status_code == 400
...
def test_admin_creates_menu_with_non_existing_restaurant(admin, client, restaurant):
    ...
    assert resp.text == '{"details":{"restaurant":["Invalid pk \\"9999\\" - object does not exist."]}}'
    assert resp.status_code == 400
...
def test_with_bad_date(client, multiple_menus):
    ...
    assert resp.status_code == 400
    assert resp.text == '{"details":"Invalid date - \'9999-99-99\'. Correct format is YYYY-MM-DD"}'
